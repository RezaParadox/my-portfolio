...
var Shader_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { expose: __expose, emit: __emit }) {
		...
		const startTelemetryWhenReady = () => {
			const checkRendering = () => {
				if (rendererInstance.value.getPerformanceStats().fps > 0) {
					telemetryCollector = startTelemetry(rendererInstance.value, "2.5.135", props.disableTelemetry, props.isPreview);
		...
		const beginRecordingResolution = async (pixelRatio) => {
			...
			const applyRatio = async (ratio, forExport) => {
				internal.setPixelRatio(ratio);
				if (typeof internal.setSize === "function") {
					const width = forExport ? evenify(cssWidth) : cssWidth;
					const height = forExport ? evenify(cssHeight) : cssHeight;
					internal.setSize(width, height, false);
				}
				await renderForCapture();
				if (forExport) await renderForCapture();
			};
			const previousForce = rendererInstance.value.setForceFullFrameRate?.(true) ?? false;
			await applyRatio(pixelRatio, true);
			return async () => {
				await applyRatio(previousRatio, false);
